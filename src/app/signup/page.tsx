...
export default function Signup () {
  ...
  const validationSchema = Yup.object().shape({
    ...
    email: Yup.string()
      ...
      .min(7),
    password: Yup.string()
      .required('Your password must be six characters long').min(6),
  ...
  return (
    ...
    <main className='m-10 h-[80vh] flex justify-center items-center'>
        <form className='signup border-2 w-[60%] h-[100%] border-black border-solid' onSubmit={handleSubmit(submitForm)}>
            <div>
            <label htmlFor="firstName">First Name:</label>
            <input type="text" /* name="username" */ id="firstName" {...register('firstName')}/>
            </div>
            <div>
            <label htmlFor="lastName">Last Name:</label>
            <input type="text" /* name="username" */ id="lastName" {...register('lastName')}/>
            </div>
            <div className='ml-28'>
            <label htmlFor="email">Email:</label>
            <input type="text" /* name="username" */ id="email" {...register('email')}/>
            </div>
            <div className='ml-4'>
