...
export default function itemDetail(req: { params: { id: any } }) {
  ...
  const submitForm = async (data: any) => {
    ...
    try {
      const req = await fetch(
        `${process.env.NEXT_PUBLIC_backend_Link}addcart`,
        {
          method: "POST",
          body: formData,
          headers: {
  ...
  useEffect(() => {
    (async () => {
      const item = await fetch(
        `${process.env.NEXT_PUBLIC_backend_Link}product/${req.params.id}`,
      );
    ...
    document.getElementById("leftBtn")?.addEventListener("click", () => {
      document
        ...
        ?.setAttribute(
          "style",
          `transform:translateX(${0}px); transition: transform 0.5s ease-in-out;`,
        );
    });
    ...
    document.getElementById("secondImg")?.addEventListener("mousedown", (e) => {
      ...
      img.setAttribute(
        "style",
        ` transform:scale(2) translate(${translateX}px, ${translateY}px); transition: transform 0.5s ease-in-out;`,
      );
  ...
  const images = imgs.map((img: any) => {
    return <img className="max-w-[100vw]" src={img} alt={product.product} />;
  });
  ...
  return (
    <>
      <Header />
      <main
        className="itemDetail h-[100%] bg-[#F6E4C8] items-center flex flex-col  "
        suppressHydrationWarning={true}
      >
        <div
          className="flex rounded-xl h-[100vh]  w-[85%] p-16 justify-around bg-[#353935] text-white"
          suppressHydrationWarning={true}
        >
          <div className="flex justify-around items-center  ">
            <button id="leftBtn">
              ...
              >
                <path
                  className="hover:fill-[#F28C28] transition-all ease-in-out"
          ...
          >
            ...
            <form
              className="w-[100%] h-[20vh] flex flex-col justify-around "
            ...
            >
              ...
              >
                ...
                <select
                  className="w-[80%] text-black text-3xl"
              ...
              <div className="flex justify-center items-center self-start ">
                ...
                <select
                  defaultValue={"1"}
                  className="w-[40%] text-black text-3xl text-center"
                  {...register("amount")}
                >
              ...
              <button
                type="submit"
                id="addToCart"
                className="w-[90%] h-[5vh] border-solid bg-white text-black rounded-xl border-black border-2 text-4xl"
              >
                Add to Cart
              </button>
            </form>
            <p className="w-[20vw] text-xl leading-loose">{product.summary}</p>
          </div>
        </div>
      </main>
      <Footer />
