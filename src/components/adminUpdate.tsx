...
export default function AdminUpdate({onChange,_id, addFlavor,product, price, flavors, stock, summary, brand, category, images, brands, categories, addImage, onSubmit }:any,){ {
const brandList = brands
const catList = categories
 
const optionBrands = brandList.map((bra: any) => {

    if(bra.name === brand.name){
...
const optionCat = catList.map((cat: any) => {
    ...
    if(cat === category){
        ...
        console.log(cat)
        return(
            <option value={cat.type} selected>{cat.type}</option>
        )
    }
    return(
...
const imageList: any = imgs.map((img: any) => {
 ...
 const itemForm = (
            ...
            <div>
                ...
                <div className="flex flex-col items-start p-10 border-b-2 border-black border-solid">
                  <label htmlFor="summary">Summary:</label>  
                    <textarea id='summary' className="border-2 border-solid border-black rounded" name="summary" data-key='summary' value={summary} onChange={onChange} cols={60} rows={10}>{summary}</textarea>
                    </div> 
    ...
    return(
        <div id = 'formUpdate' className="fixed hidden z-[20] bg-[#FFFBE9] left-[15%] top-[15%] w-[60%] p-10 border-2 border-solid border-black" >
            <button id='exitForm' onClick={() => {
